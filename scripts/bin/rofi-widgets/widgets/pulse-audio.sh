...
pulse_get_devices_polybar(){
	local colorRunningOverline="%{o$(cat "$USERCONFIG/colors/colors.json" | jq -r '.colors.color1')}"
	local colorDefaultUnderline="%{u$(cat "$USERCONFIG/colors/colors.json" | jq -r '.colors.color4')}"
	local nocolorUnderline="%{u$(cat "$USERCONFIG/colors/colors.json" | jq -r '.colors.color0')}"
