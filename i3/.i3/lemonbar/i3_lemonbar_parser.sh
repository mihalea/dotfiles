...
while read -r line ; do
	case $line in
		SYS*)
			...
			# disk /
			diskr="%{F${color_sec_b2}}${sep_left}%{F- B${color_sec_b2}} %{T2}${icon_hd}%{F- T1} ${sys_arr[6]}%"
			# disk home
			diskh="%{F-}${sep_l_left} %{T2}${icon_home}%{F- T1} ${sys_arr[7]}%"
			# wlan
		...
		VOL*)
			...
			volv=${line#???}
			volv=${volv%?}
			vol="%{F${color_sec_b2}}${sep_left}%{F- B${color_sec_b2}} %{T2}${icon_vol}%{F- T1} ${volv}"
			;;
		GMA*)
			# Gmail
			gmail="${line#???}"
		...
		MPD*)
			...
			mpd="%{F${color_sec_b2}}${sep_left}%{B${color_sec_b2}}%{F${color_sec_b2}}${sep_left}%{F- B${color_sec_b2}} %{T2}${icon_music}%{F${color_fore} T1}  ${song}"
			;;
		WSP*)
			# I3 Workspaces
		...
		BAT*)
			...
			if [[ ${bat_arr[2]} == "L" ]]; then
				...
				bkg="${color_critical}"
				frg="${color_back}"
			elif [[ ${bat_arr[2]} == "F" &&  ${bat_arr[1]} == "C" ]]; then
				bkg="${color_sec_b1}"
				frg="${c_white_l}"
			fi
		...
		# And finally, output
		printf "%s\n" "%{l}${wsp}${title} %{r}${mpd}${stab}${cpu}${stab}${mem}${stab}${diskr}${stab}${diskh}${stab}${wland}${vpn}${stab}${batamt}${stab}${vol}${stab}${date}${stab}${time}"
	done
